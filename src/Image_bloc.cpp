...
void BlocV3(int nH, int nW,OCTET* ImgIn, OCTET* ImgOut){

   int sumR;
   int sumV;
   int sumB;
    for (int i = 0; i < nH * 3  ; i += 24){
     
      for (int j = 0; j < nW * 3  ;  j += 24){ 
        sumR =0;
        sumV =0;
        sumB =0;

      for (int k = i ; k < i + 24; k += 3){
        for (int l = j ; l < j + 24; l += 3) {
         sumR += ImgIn[k * nW +l];
         sumV += ImgIn[k * nW +(l + 1)];
         sumB += ImgIn[k * nW +(l+ 2)];
        ...
        for (int k = i ; k < i + 24; k += 3){
        for (int l = j ; l < j + 24; l += 3) {
            ImgOut[k * nW +l] = (sumR / (24 * 2.5) ) > 255 ? 255 : sumR / (24 * 2.5) ;
            ImgOut[k * nW +l + 1] = (sumV / (24 * 2.5)) > 255 ? 255 : sumV / (24 * 2.5)  ;
            ImgOut[k * nW +l + 2] = (sumB / (24 * 2.5)) > 255 ? 255 : sumB / (24 * 2.5)  ;
        }}
